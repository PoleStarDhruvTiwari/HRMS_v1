...
from app.apis.access_control.permissions.routers import router as permissions_router
from app.apis.access_control.user_permissions.routers import router as user_permissions_router
from app.apis.access_control.role_permissions.routers import router as role_permissions_router
from app.apis.hierarchy.employee_hierarchy.routers import router as hierarchy_router
from app.apis.hierarchy.team_hierarchy.routers import router as team_hierarchy_router
...
def sync_permissions_on_startup() -> bool:
    try:
        ...
        if status["summary"]["in_sync"]:
            logger.info("🎉 Permissions are synchronized")
        else:
            logger.warning("⚠️ Permission drift detected")
...
async def lifespan(app: FastAPI):
    ...
    # 2️⃣ Initialize models (IMPORTS ALL TABLE DEFINITIONS)
    from app.database.base import Base, init_models
    from app.database.session import engine
    init_models()  # 🔥 VERY IMPORTANT
    # 3️⃣ Create tables
    logger.info("🗄️ Creating tables if they do not exist...")
    Base.metadata.create_all(bind=engine)
    logger.info("✅ Tables ensured")
    # 4️⃣ Run permission sync AFTER tables exist
    sync_permissions_on_startup()
    yield
    logger.info("👋 HRMS Application Shutting Down...")
